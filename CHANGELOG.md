...
## [2.7.0-DMS] - 2026-03-30

### Added
- **Data Connectors Framework (Phase 1)**: Foundation for external system integrations
  - Backend: Connector models, encrypted credential storage, CRUD APIs
  - Frontend: Data Connectors tab in Admin Tools with full management UI
  - Supports connector types: IPEC ISM, SCADA (coming soon), IoT Gateway (coming soon)
  - Features: Test connection, activate/deactivate, view mappings, view sync logs
  - Credentials encrypted using Fernet (AES) encryption

### New Files
- `/app/backend/routes_connectors.py` - Connector API endpoints
- `/app/backend/services/connectors/__init__.py` - Credential encryption
- `/app/backend/services/connectors/connector_manager.py` - Connector CRUD operations
- `/app/backend/services/connectors/base_connector.py` - Abstract connector base class
- `/app/frontend/src/components/DataConnectorsManager.jsx` - UI component

### API Endpoints
- `GET /api/connectors/types` - List available connector types
- `GET /api/connectors` - List connectors for company
- `POST /api/connectors` - Create connector
- `PUT /api/connectors/{id}` - Update connector
- `DELETE /api/connectors/{id}` - Delete connector
- `POST /api/connectors/{id}/test` - Test connection
- `POST /api/connectors/{id}/sync` - Trigger manual sync
- `GET /api/connectors/{id}/mappings` - Get device mappings
- `GET /api/connectors/{id}/sync/logs` - Get sync history

---

## [2.6.2-DMS] - 2026-03-30

### Fixed
- **Docker Build**: Fixed Dockerfile to handle missing `yarn.lock` gracefully
  - Build now succeeds whether `yarn.lock` is present or not
  - Uses `--frozen-lockfile` only when `yarn.lock` exists
